...
- v1 gated behind `hasAnkifyAccess` as a cost hedge only — not a permanent paid feature. Lift to all signed-in users once 500 calls clear the per-request token ceiling without breaching the per-conversion budget. The audience-expansion thesis (students, language learners, anyone with paper notes) assumes broad access; the gate is the unproven-spend hedge, not the product shape
...
- Mobile-native app
- Permanent paid-only positioning — the v1 gate exists to bound spend, not to position Vision as an Ankify-only feature
