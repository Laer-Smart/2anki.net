...
describe('convertDocxToHTML', () => {
  it('rejects with docx_parse_failed: prefix when given a raw PDF buffer', async () => {
    const pdfBuffer = Buffer.from(
      '%PDF-1.4 fake pdf content that is not a docx'
    );
    await expect(convertDocxToHTML(pdfBuffer)).rejects.toThrow(
      /^docx_parse_failed: /
    );
  });
  ...
  it('rejects with docx_parse_failed: prefix when given a random binary buffer', async () => {
    ...
    await expect(convertDocxToHTML(randomBuffer)).rejects.toThrow(
      /^docx_parse_failed: /
    );
  });
});
