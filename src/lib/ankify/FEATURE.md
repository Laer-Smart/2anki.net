...
- `transforms/` — pure helpers for the bulk `.apkg`-input transform pipeline. `types.ts` defines `ParsedNote`, `TransformedNote`, the `TransformName` catalog (`translate_back`, `add_example`, `cloze_front`, `add_hint`, `add_image`), the supported target-language list for `translate_back`, and the `ImageSource` enum (`pexels`, `wikimedia`) for `add_image`. `prompts.ts` builds the per-note system + user prompt for each Claude-driven transform; `add_image` does NOT use a Claude prompt — it fetches a real photo or diagram via the image-source API and appends an `<img>` tag to the back field. `applyTransformedFields.ts` maps the LLM JSON result back onto the note, flipping `modelKind` to `'cloze'` for `cloze_front`. The `TransformedNote.media` optional field carries per-note image filenames so the .apkg packager can include them in the deck's media folder (set by `add_image`, unused by the LLM-driven transforms). No Claude calls here — those live in `src/services/ankify/transformService.ts`; image fetches live in `src/services/ankify/imageFetchService.ts`. Plan gating for the transform surface uses `isPaying` (paid plan), not `hasAnkifyAccess` — Ankify and Transform are distinct paid surfaces.
