...
describe('splitByHeadings', () => {
  it('produces chunks for a single heading with a long body', () => {
    const headings: Heading[] = [
      h(
        'Polarised lenses',
        1,
        'Light passing through a polarised lens is filtered horizontally. It reduces glare from reflective surfaces. The lens material contains a special polymer. The polymer absorbs horizontal light waves selectively.'
      ),
    ];
  ...
  it('produces one chunk per heading for multiple flat headings', () => {
    const headings: Heading[] = [
      h(
        'Chapter 1',
        1,
        'First chapter has some content about topic A. More detail about topic A follows here.'
      ),
      h(
        'Chapter 2',
        1,
        'Second chapter covers topic B in detail. Even more about topic B.'
      ),
      h(
        'Chapter 3',
        1,
        'Third chapter introduces topic C. Topic C is fascinating.'
      ),
    ];
    const result = splitByHeadings(headings);
  ...
  it('emits a single chunk for a body under the short-body threshold', () => {
    ...
    const headings: Heading[] = [h('Brief section', 1, shortBody)];
    const result = splitByHeadings(headings);
    expect(result).toHaveLength(1);
  ...
  it('skips headings with empty body (parent heading in nested scenario)', () => {
    const headings: Heading[] = [
      ...
      h(
        'Child heading',
        2,
        'This child has a real body with meaningful content.'
      ),
    ];
    const result = splitByHeadings(headings);
