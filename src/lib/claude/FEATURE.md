...
**Parse hardening:** `parseDeckResponse` applies two extraction steps before `JSON.parse`. First, a line-anchored fence regex (`/^```json\n?|^```\n?|```\s*$/gm`) strips code fences from line boundaries only — backticks inside JSON string values are never touched. Second, `extractJsonArray(text)` locates the first `[` and last `]` and returns the slice, so leading/trailing prose cannot prevent parsing. Both steps run before `jsonrepair`. When the response cannot be recovered, `parseDeckResponse` throws `ClaudeParseError` (`.message === 'claude_parse_failed'`, `.name === 'ClaudeParseError'`) — the raw payload never appears in the thrown error. All three error log sites use `redactClaudePayload(text)` which emits `{ length, prefix (80 chars), sha256_prefix (12 hex) }` instead of the full payload. The `suffix` field is omitted because end-of-array carries the last card's answer text (highest leak risk).
