...
- `helpers/handleOverlappingCloze.ts` powers the `overlapping-cloze` option (`off` | `show-all` | `windowed`, gated behind `cloze`). Pure: `(items, style) => string[]` returns N note bodies, body *i* wrapping item *i* in `{{c1::…}}` and the rest rendered per style — `show-all` keeps the whole list visible as context, `windowed` keeps only one line before and one after. **Exactly one `c1` cloze per body, never `c1..cN` in one body** — Anki buries siblings, so one cloze per note is the whole point; the colocated test asserts this. `DeckParser.processPayload` selects it by style and only when a note's `back` holds a `<ul>`/`<ol>` of ≥2 `<li>` items (the Notion toggle-with-list shape); it fans the single note out into N cloze `Note`s (each `cloze = true`, body in `name`, empty `back`) mirroring the `basicReversed` fan-out. Lists under 2 items fall through to the normal cloze path silently. Never branch inside `handleClozeDeletions` for this.
