...
export interface ISubscriptionClaimTokensRepository {
  insert(
    initializer: SubscriptionClaimTokensInitializer
  ): Promise<SubscriptionClaimTokens>;
...
class SubscriptionClaimTokensRepository implements ISubscriptionClaimTokensRepository {
  ...
  async insert(
    ...
    const rows = await this.database('subscription_claim_tokens')
      .insert(initializer)
      .returning('*');
    return rows[0];
  }
  ...
  async findByTokenHash(
    tokenHash: string
  ): Promise<SubscriptionClaimTokens | null> {
    const row = await this.database('subscription_claim_tokens')
  ...
  async markConsumed(id: number, trx: Knex.Transaction): Promise<void> {
    await trx('subscription_claim_tokens')
      .where({ id })
