...
| **OPML** (`.opml`) | Each parent→child outline relationship becomes one Basic card: the parent text is the front, the child text is the back. | Same size limit as any upload. | Export from OmniOutliner, iThoughtsX, or any outliner that supports OPML. |
| **Brainstorms JSON** (`.brainstorms.json`) | Same parent→child edge rule as OPML — each edge becomes one Basic card. | Same size limit as any upload. | Export from Brainstorms (iOS/macOS) using the JSON export option. The file must end in `.brainstorms.json`. |
