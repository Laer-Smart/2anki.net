...
- **A fresh worktree has no toolchain — install before doing anything, and don't hand-patch native binaries.** `git worktree add` / `EnterWorktree` gives you source only: no `node_modules`, and even after `pnpm install` the Oxc native addons (`@oxlint/binding-<platform>`, `@oxfmt/binding-<platform>`) may be missing because pnpm skipped the optional platform dep, so `oxfmt`/`oxlint` die with `Cannot find native binding` / `MODULE_NOT_FOUND`. **First action in any worktree: `pnpm install && pnpm --filter 2anki-web install`** (both workspaces — web has its own `node_modules`). If the Oxc binding is still absent, re-run `pnpm install --force` or `pnpm rebuild` — **do NOT** `npm pack` the native `.node` and copy it into `node_modules/oxfmt/dist/` by hand, and do NOT `pnpm add -w @oxfmt/...` (it dirties `package.json`/lockfile, which you then have to revert). One 2026-06-15 incident burned ~10 tool calls manually fetching the darwin-arm64 oxfmt binary; the fix is to install up front, not to MacGyver the binary mid-session.
